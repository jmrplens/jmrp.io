...
| 11  | SonarCloud Analysis   | `pnpm exec sonar-scanner` _(requires `SONAR_TOKEN`)_                                    |
| 12  | SonarCloud Issues     | `node scripts/ci/get-sonar-issues.mjs` _(requires `SONAR_TOKEN` + `SONAR_PROJECT_KEY`)_ |
...
- **Headers**: Security headers are generated by `post-build.ts` into `.nginx-staged/security_headers.conf` and moved to `/etc/nginx/snippets/jmrp/` by `deploy-live.mjs` after the deploy swap — they are never part of the served output. Change them in `src/integrations/post-build/csp.ts`, never in the generated file: the staged copy is overwritten by the next build and the delivered copy is replaced by the next deploy.
