...
<div style="padding: 0;">${body}</div>
  ...
  // 7. CompareCode: <CompareCode badTitle="..." goodTitle="..."> ... </CompareCode>
  flattened = flattened.replaceAll(
  ...
  return rss({
    ...
    items: await Promise.all(
      publishedPosts.map(async (post: CollectionEntry<"posts">) => {
        ...
        let contentWithImage = sanitizedHtml;
        ...
        if (post.data.coverImage) {
          try {
            ...
            // Prepend image to content (Best practice for compatibility)
            contentWithImage = `<img src="${imageUrl}" alt="${post.data.title}" width="${optimizedImage.attributes.width}" height="${optimizedImage.attributes.height}" style="display: block; margin-bottom: 24px; border-radius: 8px; max-width: 100%; height: auto;" /><br/>${sanitizedHtml}`;
