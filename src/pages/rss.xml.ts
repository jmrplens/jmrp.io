  ...
  return rss({
    ...
    items: await Promise.all(
      publishedPosts.map(async (post) => {
        ...
        const html = await marked.parse(await flattenComponents(post.body || ""));
        const cleanHtml = sanitizeHtml(html, {
          ...
          allowedSchemesByTag: { img: ["http", "https", "data"], source: ["data"] },
        });

        const styledHtml = juice(cleanHtml, { extraCss: RSS_STYLES, applyStyleTags: false, removeStyleTags: true, preserveMediaQueries: true, preserveFontFaces: false, insertPreservedExtraCss: false });
