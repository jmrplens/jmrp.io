...
<Mermaid caption="Nginx mTLS Handshake Flow" maxWidth="400px">
```mermaid-render
sequenceDiagram
...
<FileContent filename="ca.key" icon="mdi:key-variant" collapsible>
-----BEGIN PRIVATE KEY-----
...
<FileContent filename="ca.crt" icon="mdi:certificate-outline" collapsible>
-----BEGIN CERTIFICATE-----
...
2fXleID8q60zmJik0fcTyVoBdEg6KWVAykEia+E=
-----END CERTIFICATE-----
</FileContent>


You will be asked for details like Country, State, etc. Fill these out for your organization. The "Common Name" (CN) is important—set it to something like `jmrp.io-CA`.

To verify your new CA certificate:
<TerminalCommand>
...
<FileContent filename="user.key" icon="mdi:key-variant" collapsible>
-----BEGIN PRIVATE KEY-----
...
<FileContent filename="user.crt" icon="mdi:certificate-outline" collapsible>
-----BEGIN CERTIFICATE-----
...
<FileContent filename="/etc/nginx/sites-available/secure.example.com.conf" icon="devicon:nginx" ariaLabel="Nginx Config: Client Certificate Verification">
```nginx
server {
...
<FileContent filename="/etc/nginx/sites-available/secure.example.com.conf" icon="devicon:nginx" ariaLabel="Nginx Config: Advanced Access Control">
```nginx
server {
    # ... ssl configuration ...
...
<FileContent filename="/etc/nginx/sites-available/secure.example.com.conf" icon="devicon:nginx" ariaLabel="Nginx Config: CRL Revocation">
```nginx
# inside server block
