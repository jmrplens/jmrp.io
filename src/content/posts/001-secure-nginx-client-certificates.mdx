...
In this guide, I'll walk you through creating a bespoke Certificate Authority, generating client certificates, configuring Nginx, and handling advanced scenarios like **revoking access** for lost devices or restricting specific users. You can find more deep-dive tutorials from providers like <a href="https://smallstep.com/hello-mtls/" target="_blank" rel="noopener noreferrer" aria-label="Read Smallstep's deep-dive tutorial on mTLS">Smallstep</a>.
...
- The [`openssl`](https://www.openssl.org/docs/manpages.html) command-line tool installed (standard on almost all Linux distros).
...
<TerminalCommand command="mkdir -p /etc/nginx/client-certs && cd /etc/nginx/client-certs" prompt="#"/>
<TerminalCommand command="mkdir -p newcerts && touch index.txt && echo 1000 > serial" prompt="#"/>
