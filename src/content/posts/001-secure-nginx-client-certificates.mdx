...
<FileContent filename="/etc/nginx/client-certs/openssl.cnf">
  ```ini [ ca ] default_ca = CA_default [ CA_default ] dir = . database =
  $dir/index.txt new_certs_dir = $dir/newcerts certificate = $dir/ca.crt serial
  = $dir/serial private_key = $dir/ca.key default_days = 365 default_crl_days=
  30 default_md = sha256 policy = policy_loose copy_extensions = copy [
  ...
  optional commonName = supplied emailAddress = optional [ req ]
  distinguished_name = req_distinguished_name prompt = no [
  req_distinguished_name ] C = ES ST = Valencia L = Valencia O = JMRP-IO-LAB OU
  = IT Services CN = JMRP-IO-LAB Root CA ```
</FileContent>
