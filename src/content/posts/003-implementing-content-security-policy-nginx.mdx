...
**Cross-Site Scripting (XSS)** is one of the most dangerous web vulnerabilities. Attackers inject malicious scripts that steal cookies, redirect users, or perform actions on their behalf. **Content Security Policy (CSP)** is your best defense—a whitelist that tells the browser exactly which resources are allowed to load and execute.

In this guide, you'll learn CSP from the ground up. We'll start with a one-line policy, then progressively build toward a strict, A+ rated configuration. By the end, you'll understand not just *how* to implement CSP, but *why* each directive matters.
...
flowchart LR
    ...
    A["Your scripts"]:::success --> CSP
    B["CDN scripts"] --> CSP
    C["Inline scripts"] --> CSP
    D["Malicious scripts"]:::danger --> CSP
    
    CSP -->|"Allowed"| E["Execute"]:::success
    CSP -->|"Blocked"| F["Reject"]:::danger
```
</Mermaid>
...
For complete documentation, refer to the official MDN references. Below are the key directive categories:
...
These control document behavior and navigation:
...
Let's construct a complete CSP progressively. Each level adds more protection.
...
<Callout type="success">
  **Checkpoint:** This CSP protects against most attacks. But there's one problem—inline scripts are blocked. If your site uses any inline JavaScript, it won't work. Let's fix that.
</Callout>
...
Here's the challenge: Static Site Generators (Astro, Next.js, Hugo) build HTML at **build time**. They can't know the random nonce that should be generated at **request time**.

### The solution: placeholder substitution

1. At build time, use a placeholder string
2. At request time, Nginx replaces the placeholder with a real nonce
...
flowchart TB
    subgraph CSP["How strict-dynamic Works"]
        direction TB
        N["Nonce in CSP header<br/>nonce-abc123"]
    end
    
    subgraph Trusted["Trusted Execution Chain"]
        direction LR
        A["Script tag with<br/>nonce=abc123"]:::success
        B["createElement script<br/>(no nonce needed)"]:::success
        C["Loaded library<br/>(inherits trust)"]:::success
        A -->|"Creates"| B
        B -->|"Loads"| C
    end
    ...
    subgraph Blocked["Blocked by CSP"]
        direction LR
        D["Injected inline<br/>script tag"]:::danger
        E["Event handler<br/>onclick=..."]:::danger
    end
    
    N -->|"Validates"| A
    D -->|"No match"| X["Blocked"]:::danger
    E -->|"No match"| X
```
</Mermaid>
...
Based on [Google's research](https://web.dev/articles/strict-csp), a "strict CSP" requires:

<Callout type="keypoint" icon="mdi:check-decagram">
  **What makes a CSP "strict":**
  
  1. Uses **nonces** or **hashes** instead of domain allowlists
  2. Includes **`'strict-dynamic'`** for dynamic script loading
...
You don't need the same CSP everywhere. A common pattern is:

- **Strict CSP** for HTML pages (maximum protection)
- **Relaxed CSP** for static assets (allow crawlers to access images)

### Why relax CSP for assets?

Social media crawlers and link preview tools need to access your images. If your CSP is too strict on asset endpoints, platforms like Twitter, Facebook, or Slack won't be able to show preview images.
...
Never deploy a strict CSP directly to production. Use this gradual approach:
...
3. **Missing nonces** → Add `nonce="CSP_NONCE_NGINX"` to inline scripts
...
When CSP blocks something, browsers can send a report. This helps you:

- Detect real attacks
- Find bugs in your policy
...
<Code lang="nginx" aria-label="report-uri directive">
```nginx
report-uri /csp-reports;
```
</Code>
