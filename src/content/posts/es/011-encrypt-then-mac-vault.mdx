...
faq:
  ...
  - question: "¿Por qué AES-256-CBC más HMAC en lugar de AES-GCM?"
    answer: "Por uniformidad de backend, no por preferencia criptográfica: en un protocolo nuevo GCM o ChaCha20-Poly1305 serían la elección obvia. El mismo código debe producir una salida byte a byte idéntica en la compilación de host contra la API antigua de Mbed TLS y en el dispositivo contra la API PSA Crypto de Mbed TLS 4.0. CBC, HMAC y una comparación en tiempo constante son deterministas en ambos; las reglas de gestión de nonces de GCM entre dos backends son un riesgo mayor. Encrypt-then-MAC es la composición genéricamente segura, que es como IPsec y KDBX 4 construyen también su AEAD."
...
La bóveda no escribe ese bucle a mano: llama a [`mbedtls_ct_memcmp`](https://mbed-tls.readthedocs.io/projects/api/en/development/api/file/constant__time_8h/) de Mbed TLS —el primitivo de comparación en tiempo constante de la librería—, que hace exactamente esto. Si nunca has visto por qué importa, [_A Lesson In Timing Attacks_](https://web.archive.org/web/2023/https://codahale.com/a-lesson-in-timing-attacks/) de Coda Hale es la clásica explicación de cinco minutos; las [notas sobre tiempo constante de BearSSL](https://www.bearssl.org/constanttime.html) van más a fondo.
...
### El verificador de PIN: comprobar el PIN sin almacenarlo
