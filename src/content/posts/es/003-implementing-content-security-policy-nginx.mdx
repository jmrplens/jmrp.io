...
Medida con la API de Mozilla Observatory, esta configuración obtiene una **A+**. La calificación se ha mantenido en las dos generaciones de puntuación: 140 con los 10 tests pasados bajo el algoritmo v5, y 160 con los 12 pasados bajo v6, medido el 2 de septiembre de 2026. Ninguna de las dos es el 145 que el README de este repositorio anunció hasta hace poco, de un commit de diciembre de 2025. Aquel total incluía un bonus de 5 puntos que Observatory concede por servir cookies con `Secure`/`HttpOnly`/`SameSite`; esas cookies eran dos valores dummy que no hacían nada útil, y eliminarlas —para que la página `/privacy/` de este sitio pueda afirmar honestamente que ninguna respuesta fija `Set-Cookie`— costó 5 puntos cosméticos en una política que ya no falla ni un solo test. Ese cambio mereció claramente la pena, y el badge ahora lleva la calificación y no una puntuación, porque la calificación es la parte que no se mueve.
...
## ¿Cómo funcionan los ataques XSS y cómo los detiene CSP?

Un ataque XSS consigue que el navegador de la víctima ejecute un script elegido por el atacante dentro del origen de tu página, normalmente a través de una entrada que la página devolvió sin escapar. CSP lo detiene porque el navegador se niega a ejecutar cualquier script que la política no autorice, así que el código inyectado llega a la página y aun así nunca se ejecuta.
...
## ¿Qué ataques previene realmente una CSP?
...
## ¿Por qué los scripts inline son la parte difícil de CSP?

Los scripts inline son la parte difícil porque una política estricta bloquea todo bloque `<script>` que no lleve nonce ni hash, y la vía de escape habitual, `'unsafe-inline'`, reabre justo la puerta por la que entra el XSS. Cada bloque inline tiene que llevar un nonce por petición o pasar a un fichero externo, y ese es el trabajo que la mayoría de sitios subestima.
...
### ¿Cuándo conviene usar `'strict-dynamic'`?
...
### ¿Por qué no funcionan las allowlists?
...
## ¿Qué falla más a menudo al adoptar CSP?

Los cinco errores más frecuentes son permitir `'unsafe-inline'` para scripts, listar orígenes tan amplios que también permiten el host del atacante, omitir una directiva que la política necesita (`base-uri`, `object-src`, `form-action`), olvidar el `always` en el `add_header` de Nginx de modo que las páginas de error salen sin cabecera, y perder la cabecera por la herencia de `add_header` en un bloque anidado. Todos dejan la cabecera en el cable y quitan la protección.
...
<Callout type="success" icon="mdi:trophy">
  **Resultado:** Esta es exactamente la configuración que corre en producción en este sitio — una calificación A+ en [Mozilla Observatory](https://developer.mozilla.org/en-US/observatory), con la funcionalidad completa del sitio intacta y los usuarios protegidos de ataques XSS.
</Callout>
