...
lead: "El 94,72 % de las CSP basadas en listas de permitidos se pueden eludir, según la propia investigación de Google; lo que de verdad detiene el script inyectado —y saca un A+— es una política con nonce y strict-dynamic."
...
<TLDRSummary title="TL;DR — una CSP basada en nonces sobre Nginx">
  - **Las allowlists no aguantan.** La propia investigación de Google encontró que el **94,72 % de las CSP basadas en allowlist se pueden eludir**, porque los CDN de confianza alojan endpoints JSONP y librerías desactualizadas que convierten un origen permitido en una vía de ejecución.
  - **Un nonce es por petición e impredecible**, así que un `<script>` inyectado no tiene token válido y nunca se ejecuta — que es justo lo que una allowlist no puede prometer.
  ...
  - **El resultado medido:** una **A+ en Mozilla Observatory con 140 puntos y los 10 tests superados**, alcanzada tras ejecutar `Content-Security-Policy-Report-Only` con `report-uri` durante una o dos semanas antes de aplicarla.
</TLDRSummary>
