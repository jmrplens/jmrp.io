...
lead: "El 75,81 % de las CSP usan listas de permitidos de scripts que un atacante puede eludir, según la propia investigación de Google; lo que de verdad detiene el script inyectado —y saca un A+— es una política con nonce y strict-dynamic."
...
faq:
  ...
  - question: "¿Por qué los nonces son mejores que las allowlists de dominios en CSP?"
    answer: "La investigación de Google logró eludir el 94,72 % de todas las políticas CSP distintas, y el 75,81 % se apoya en listas de permitidos de scripts que un atacante puede eludir, porque los CDN de confianza suelen alojar endpoints JSONP, redirecciones abiertas o script gadgets que los atacantes explotan. Los nonces son tokens criptográficamente aleatorios por petición que un atacante no puede adivinar, así que ofrecen protección real contra XSS sin importar qué bibliotecas se carguen."
...
howto:
  ...
  steps:
    - name: "Añadir una cabecera CSP inicial"
      ...
      anchor: "tu-primera-csp-en-5-minutos"
    ...
    - name: "Construir una política base de Nivel 6"
      ...
      anchor: "construyendo-tu-csp-capa-a-capa"
    - name: "Inyectar nonces por petición para scripts inline"
      ...
      anchor: "nonces-para-sitios-estáticos-el-truco-de-nginx"
    - name: "Pasar a modo estricto con nonces y strict-dynamic"
      ...
      anchor: "modo-estricto-con-strict-dynamic"
    ...
    - name: "Analizar y corregir las violaciones"
      ...
      anchor: "configurar-el-reporting-de-csp"
...
<TLDRSummary title="TL;DR — una CSP basada en nonces sobre Nginx">
...
{/* El <span> dentro de este <code> es FUNCIONAL, no una errata. Esta frase nombra el marcador
    que usa producción de verdad, y el propio `sub_filter` de producción reescribe ese literal
    al servir — escribirlo contiguo hacía que la página publicara un nonce en vivo en lugar del
    token (auditoría GEO 2026-08-22, K1). Partirlo deja el texto renderizado y copiado idéntico
    mientras los bytes servidos nunca coinciden. Con backticks NO funciona, y `&#67;` tampoco:
    cheerio vuelve a decodificar las entidades — verificado. En el resto del post el token se
    escribe deliberadamente invertido, como CSP_NONCE_NGINX. */}

Esto no es hipotético — jmrp.io es en sí mismo la implementación de referencia de todo lo anterior. En producción, Nginx fija `set $cspNonce $request_id;` y reescribe el marcador con <code>sub_filter NGINX_CSP_<span>NONCE</span> $cspNonce;</code>, de modo que cada respuesta lleva un nonce fresco atado a esa petición. La cabecera que este sitio sirve realmente está basada en nonces: `default-src 'none'`, `script-src 'self' 'nonce-…' 'strict-dynamic'`, `style-src 'self' 'nonce-…'`, más `frame-ancestors 'none'`, `base-uri 'none'` y `report-uri /csp-report`. El `'self'` que aparece ahí es un respaldo de mismo origen para navegadores sin soporte de `strict-dynamic`; donde sí lo soportan, el navegador ignora `'self'` y el nonce es lo único que autoriza la ejecución. Cero `unsafe-inline`, cero listas de dominios de terceros que mantener.
...
### Qué contienen de verdad 81 días de informes de violación

Entre el 8 de junio y el 27 de agosto de 2026 —81 días naturales— ese receptor escribió en disco **1.756 informes de violación**. Repartidos entre esos 81 días salen 21,7 al día, y la media es el número menos útil de todo el conjunto: el día mediano trae 4 informes, dieciséis días no trajeron ninguno y el 1 de agosto, él solo, trajo 371.

Pasar todo el histórico por los filtros que el receptor aplica *hoy* lo ordena así. Es una clasificación retroactiva —los filtros se fueron afinando durante la ventana—, de modo que esto es cómo lee la lógica actual ese histórico, no cómo se trató cada informe en su momento.

<Table title="1.756 informes CSP, 8 de junio – 27 de agosto de 2026">
  <thead>
    <tr>
  ...
  <tbody>
    <tr>
      <td>User-agent de crawler</td>
      <td>1.399</td>
      <td>79,7 %</td>
    </tr>
    <tr>
      <td>Residual — revisado a mano</td>
      <td>287</td>
      <td>16,3 %</td>
    </tr>
    <tr>
      <td>Rango de red de crawler</td>
      <td>60</td>
      <td>3,4 %</td>
    </tr>
    <tr>
      <td>Extensión del navegador / style inline inyectado</td>
      <td>10</td>
      <td>0,6 %</td>
    </tr>
  </tbody>
...
El cubo residual es el que merece la pena leer, y son cuatro cosas: **247** informes de un `eval` bloqueado, **29** de un script inline bloqueado, **8** fallos de SRI sobre fragmentos `/_astro/` con hash y **3** peticiones bloqueadas a un píxel de analítica de Yandex inyectado.

Ninguna de ellas es un defecto del sitio. Los informes de `eval` son el caso más claro: ningún fichero JavaScript que este sitio sirve contiene `eval(` ni `new Function(` — tras una compilación de producción, las únicas apariciones de ese literal en toda la salida están en el texto de este mismo post. Esos 247 informes vienen de **5 direcciones IP en 5 días**, 223 de ellos de un único user-agent, y la página que citan es casi siempre `/homelab/` (178) o la portada (45). Eso es un visitante con algo inyectando código en su navegador. Los 29 informes de script inline tienen la misma forma: 4 direcciones, 4 días y ni un `source-file`.

### El embudo: lo que se descarta antes de llegar al log

El log es solo la mitad de la foto, porque los informes que los filtros descartan no llegan a escribirse. Su único rastro es un contador en memoria que el proceso vuelca al journal cada 100 descartes — y en una ventana concreta, del 22 al 27 de agosto (4,67 días), ese contador y el log se pueden cruzar: el `no-notify:crawler-ua=97` del contador coincide, exactamente, con los 97 informes de crawler que el log guarda en esa misma ventana. Eso da el embudo completo.

<Table title="Embudo de informes CSP, 22–27 de agosto de 2026 (4,67 días)">
  <thead>
    <tr>
  ...
  <tbody>
    <tr>
      <td>Superan el parseo JSON y las guardas defensivas</td>
      <td>313</td>
      <td>100 %</td>
    </tr>
    <tr>
      <td>Descartados antes de llegar al log</td>
      <td>213</td>
      <td>68,1 %</td>
    </tr>
    <tr>
      <td>— extensión del navegador o antivirus</td>
      <td>172</td>
      <td>55,0 %</td>
    </tr>
    <tr>
      <td>— respaldo de prefetch</td>
      <td>14</td>
      <td>4,5 %</td>
    </tr>
    <tr>
      <td>— style inline inyectado</td>
      <td>14</td>
      <td>4,5 %</td>
    </tr>
    <tr>
      <td>— contexto interno del navegador</td>
      <td>7</td>
      <td>2,2 %</td>
    </tr>
    <tr>
      <td>— recurso de terceros inyectado</td>
      <td>6</td>
      <td>1,9 %</td>
    </tr>
    <tr>
      <td>Crawlers: se registran, nunca avisan</td>
      <td>97</td>
      <td>31,0 %</td>
    </tr>
    <tr>
      <td>Llegan a la vía de notificación</td>
      <td>3</td>
      <td>1,0 %</td>
    </tr>
    <tr>
      <td>Defectos del propio sitio</td>
      <td>0</td>
      <td>0 %</td>
    </tr>
  </tbody>
...
Los tres que llegaron a la vía de notificación eran el mismo píxel de analítica de Yandex inyectado, bloqueado tres veces en menos de 200 ms en una sola página y para un solo visitante.

Con esa tabla van dos advertencias, y ambas importan más que los porcentajes.

**313 es una cota inferior, no un total.** Los cuerpos que fallan al parsear el JSON (un 400) o que superan el tope de 32 KB (un 413) se rechazan antes de que ningún contador los vea, y dos `return` defensivos dentro del manejador de informes tiraban los cuerpos malformados sin contarlos tampoco. Así que la etiqueta honesta es «informes que superaron el parseo y las guardas», no «informes recibidos». Desde entonces he enganchado esas rutas al mismo contador, que es justo el tipo de agujero que solo aparece cuando intentas escribir la cifra.

**Los dos ritmos de este post no son comparables.** 313 informes en 4,67 días son 67 al día, frente a los 21,7 al día del log de 81 días — porque el log no contiene ni un descarte. Las dos cifras son ciertas y miden cosas distintas.

### El fallo que esta auditoría encontró en mi propio receptor

Estos números solo son correctos porque comprobarlos destapó un fallo de parseo en el propio receptor.
...
<Code lang="javascript" title="scripts/csp-reporter.mjs — el fallo">
```javascript
...
Lo importante es el tamaño de la corrección. Antes de descodificar esos diez, el embudo de arriba decía «13 informes llegan a la vía de notificación» — y el «cero defectos reales» que iba al lado se apoyaba en diez informes que nadie había leído nunca. La cifra verdadera es 3. Reproducir los 81 días completos por la vía de notificación actual da **26 alertas, una cada 3,1 días** (7 por `eval`, 8 por scripts inline, 8 por SRI sobre fragmentos `/_astro/` caducados y 3 por el píxel de Yandex); reproducirlos con el fallo todavía dentro da 30, y las cuatro de más van vacías.

<Callout type="keypoint" title="Un informe que no sabes parsear es peor que uno que nunca recibiste">
...
<Timeline events={[
  { date: "2010", type: "default", title: "X-Content-Security-Policy", description: "Mozilla presenta la primera implementación de CSP como cabecera experimental. [Blog de Seguridad de Mozilla](https://blog.mozilla.org/security/2009/06/19/shutting-down-xss-with-content-security-policy/)" },
  ...
  { date: "2016", type: "warning", title: "Investigación de Google", description: "Google publica 'CSP Is Dead, Long Live CSP' demostrando que se puede eludir el 94,72 % de todas las políticas distintas, de las cuales el 75,81 % usa listas de permitidos de scripts eludibles. [Artículo de investigación](https://research.google/pubs/csp-is-dead-long-live-csp-on-the-insecurity-of-whitelists-and-the-future-of-content-security-policy/)" },
  { date: "2018", type: "success", title: "strict-dynamic", description: "Nueva keyword que permite a los scripts de confianza cargar dependencias sin necesidad de allowlist explícito. [MDN strict-dynamic](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/script-src#strict-dynamic)" },
...
<FileContent filename="nginx.conf (CSP con Trusted Types)">
```nginx
add_header Content-Security-Policy "
    default-src 'none';
    script-src 'self' 'nonce-$cspNonce' 'strict-dynamic';
...
<FileContent filename="Cabecera CSP con reporting">
```nginx
...
<Table title="Herramientas de prueba de CSP">
  ...
  <tbody>
    <tr>
      <td><a href="https://developer.mozilla.org/en-US/observatory" target="_blank" rel="external noopener noreferrer" class="external-link" aria-label="Mozilla Observatory (se abre en nueva pestaña)"><strong>Mozilla Observatory</strong></a></td>
      <td>Evaluación completa de cabeceras de seguridad (A+ posible)</td>
    </tr>
    <tr>
      <td><a href="https://csp-evaluator.withgoogle.com/" target="_blank" rel="external noopener noreferrer" class="external-link" aria-label="Google CSP Evaluator (se abre en nueva pestaña)"><strong>Google CSP Evaluator</strong></a></td>
      <td>Encuentra bypasses lógicos en tu política (muy recomendado)</td>
    </tr>
    <tr>
      <td><a href="https://securityheaders.com/" target="_blank" rel="external noopener noreferrer" class="external-link" aria-label="SecurityHeaders.com (se abre en nueva pestaña)"><strong>SecurityHeaders.com</strong></a></td>
      <td>Análisis rápido de cabeceras y puntuación</td>
    </tr>
    <tr>
      <td><a href="https://report-uri.com/tools/csp-hash-generator" target="_blank" rel="external noopener noreferrer" class="external-link" aria-label="CSP Hash Generator (se abre en nueva pestaña)"><strong>CSP Hash Generator</strong></a></td>
