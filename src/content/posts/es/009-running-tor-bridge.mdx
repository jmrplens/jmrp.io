...
## ¿Qué tipo de nodo de Tor puedes operar como voluntario?
...
<KeyValue
  ...
  items={[
    ...
    { key: "ORPort [IPv6]:9001", value: "Onion Router Port (IPv6)", description: "Si tu servidor tiene una dirección IPv6 global, añade una segunda línea ORPort con ella. Las direcciones IPv6 son enrutables globalmente: no hace falta NAT ni reenvío de puertos, solo reglas de firewall." },
    ...
    { key: "MaxMemInQueues 4096 MB", value: "Límite de memoria para colas", description: "Limita la memoria que Tor dedica a sus colas internas. Evita que el sistema mate el proceso por falta de memoria (OOM) en servidores con poca RAM. Ajusta el valor a la memoria de la que dispongas." },
...
A diferencia de IPv4, las direcciones IPv6 son **enrutables globalmente**: no hay NAT. Eso quiere decir que la dirección IPv6 de tu servidor es accesible desde Internet sin reenvío de puertos. Aun así, sigue haciendo falta:

1. **Permitir los puertos en el firewall del servidor** (UFW cubre IPv4 e IPv6 a la vez)
2. **Permitir los puertos en el firewall IPv6 del router** — casi todos los routers llevan un firewall IPv6 aparte que bloquea por defecto todo el tráfico entrante

<Callout type="warning" title="El firewall IPv6 del router">
Casi todos los routers **bloquean todo el tráfico IPv6 entrante** por defecto (a diferencia de IPv4, donde el "firewall" lo hace el propio NAT). Tienes que crear **reglas de permiso** explícitas en el firewall IPv6 del router para los puertos TCP 9001 y 4443; sin ellas nada llega a la dirección IPv6 de tu servidor. No hace falta NAT de destino: bastan reglas de aceptación en el firewall.
</Callout>

<TerminalCommand ariaLabel="Comprobar las reglas del firewall IPv6">
```bash
...
## ¿Cómo encajan entre sí las piezas del bridge?

El bridge es un único proceso de Tor alcanzable por dos pluggable transports, obfs4 y WebTunnel, que el router expone con redirecciones de puerto IPv4 y reglas de cortafuegos IPv6 mientras las reglas de exclusión de CrowdSec mantienen esos puertos abiertos; los dos transports entregan sus conexiones a Tor por ExtORPort, Tor habla hacia fuera con la red, y Prometheus lee el MetricsPort del que se alimenta Grafana.
...
## ¿Cómo confirmar que el bridge funciona de verdad?
...
<li data-check="optional">Accesible por IPv6 desde fuera: `nc -6zv YOUR_IPV6 9001`</li>
<li data-check="optional">El bypass de CrowdSec funciona (si procede): `cscli explain` con una línea de log de prueba</li>
</CheckList>

---

## ¿Qué necesita un bridge una vez está en marcha?

Un bridge en marcha necesita tres cosas de ti: paciencia, porque el arranque, la publicación del descriptor y la distribución por BridgeDB tardan uno o dos días y la bandera Stable alrededor de una semana; monitorización, porque las métricas que expone son la única forma de saber que sigue ayudando a alguien; y mantenimiento rutinario, es decir, actualizaciones, disponibilidad y una comprobación ocasional de que sus puertos siguen siendo alcanzables desde fuera.
