...
faq:
  ...
  - question: "¿Por qué usar un prefix ULA y NAT66 para IPv6 en lugar de direcciones globales?"
    answer: "Los clientes VPN reciben direcciones privadas Unique Local (ULA) del rango fd00::/8, que no entran en conflicto con direcciones IPv6 públicas. El router las traduce luego mediante NAT66 al prefix IPv6 global asignado por tu ISP cuando los clientes acceden a internet."
  - question: "¿Por qué usar un port no estándar como 53537 en lugar del 51820?"
    answer: "Usar un port no estándar en lugar del 51820 por defecto ayuda a evitar escaneos automatizados dirigidos a puertos VPN conocidos. Algunas redes también bloquean puertos VPN estándar, así que un port alto aleatorio puede ofrecer mejor conectividad."
  ...
  - question: "¿Funciona el IPv6 por la VPN si mi ISP no proporciona IPv6?"
    answer: "No hay acceso a internet por IPv6 global. NAT66 traduce la dirección ULA del cliente al prefix IPv6 global de tu ISP, así que sin un prefix del ISP no hay nada a lo que traducir. El tunnel sigue transportando IPv6 entre cliente y router, pero no más allá."
topics:
  - name: "WireGuard"
    wikidata: "Q28975568"
  - name: "MikroTik"
...
howto:
  ...
  tools:
    ...
    - "Cliente WireGuard"
  steps:
    - name: "Crear la interface WireGuard"
      text: "Añade una interface WireGuard en el router MikroTik con un port de escucha personalizado; RouterOS genera el par de claves automáticamente. Recupera la clave pública del servidor para configurar los clientes después."
      anchor: "paso-1-crear-la-interface-wireguard"
    - name: "Asignar direcciones IPv4 e IPv6"
      text: "Asigna a la interface WireGuard una dirección IPv4 y una dirección ULA IPv6 para que actúe como gateway de los clientes VPN. Establece advertise=no en la dirección IPv6."
      anchor: "paso-2-asignar-direcciones-ip-a-la-interface"
    - name: "Añadir la interface a las listas de interfaces"
      text: "Añade la interface WireGuard a las listas VPN y LAN para que las reglas de firewall traten su tráfico correctamente y los clientes accedan a los recursos locales."
      anchor: "paso-4-añadir-la-interface-a-las-listas-de-interfaces"
    - name: "Configurar las reglas de firewall"
      text: "Añade reglas de input para aceptar WireGuard en su port UDP, reglas de forward para permitir el acceso de los clientes a internet y a la LAN doméstica, y una address list para la red WireGuard."
      anchor: "paso-5-configurar-reglas-de-firewall"
    - name: "Configurar NAT/Masquerade"
      text: "Añade reglas srcnat de masquerade IPv4 y NAT66 IPv6 para que las direcciones privadas de los clientes VPN se traduzcan a tu IP pública y al prefix IPv6 global asignado por tu ISP."
      anchor: "paso-6-configurar-natmasquerade"
    - name: "Ajustar el MSS clamping y la tabla RAW"
      text: "Añade una regla mangle change-mss para evitar la fragmentación por el tunnel y reglas RAW en prerouting para omitir la protección antiflood del tráfico WireGuard."
    ...
    - name: "Configurar el cliente y verificar la conexión"
      text: "Configura cada dispositivo cliente mediante código QR o un fichero de configuración manual, y luego verifica el handshake en ambos extremos y prueba la conectividad IPv4 e IPv6 por el tunnel."
...
<TLDRSummary title="TL;DR — WireGuard dual-stack en RouterOS">
  ...
  - **El IPv6 va por ULA más NAT66, no por prefix delegado.** Los clientes reciben direcciones `fd00::/8` que el router traduce al prefix global de tu ISP, de modo que el túnel sobrevive a que ese prefix cambie por debajo.
  ...
  - **El split tunnel se decide en el `AllowedIPs` del cliente**, no en el servidor: lista solo los prefixes de casa para encaminar el tráfico doméstico, o `0.0.0.0/0, ::/0` para encaminarlo todo.
...
- **`wg_devices`** — nombre de la interface WireGuard (personalízalo si prefieres otro nombre)
- **`53537`** — port de escucha de WireGuard (elige cualquier port UDP no utilizado)
- **`192.168.100.0/24`** — subnet IPv4 de la VPN (cámbiala si entra en conflicto con tu red)
- **`fd00:1111:2222:100::/64`** — prefix ULA IPv6 de la VPN (genera el tuyo en [unique-local-ipv6.com](https://www.unique-local-ipv6.com/))
- **`192.168.0.0/24`** — subnet de tu LAN doméstica (ajústala a tu red)
...
La configuración de arriba no es un ejercicio de laboratorio — es el WireGuard que corre hoy en mi propio RB5009UG+S+ con RouterOS 7.23.2, con `wg_devices` en el port UDP 53537 exactamente como se usa a lo largo de esta guía, sirviendo a 3 peers activos con la MTU estándar de 1500.
...
Utilizamos [direcciones Unique Local (ULA)](https://datatracker.ietf.org/doc/html/rfc4193) del rango `fd00::/8` para la VPN. Estas direcciones son privadas (como RFC1918 para IPv4) y no entran en conflicto con direcciones IPv6 públicas. El router las traduce mediante NAT66 al prefix IPv6 global asignado por tu ISP.
</Callout>
...
<Prerequisite title="Antes de empezar">
  ...
  - **Capacidad de port forwarding UDP** si tu MikroTik está detrás de otro router/NAT
...
<Code lang="routeros" title="Crear interface WireGuard">
```routeros
...
<Callout type="tip" title="Selección de port">
Usar un port no estándar como `53537` en lugar del `51820` por defecto ayuda a evitar escaneos automatizados dirigidos a puertos VPN conocidos. Algunas redes también bloquean puertos VPN estándar, así que un port alto aleatorio puede ofrecer mejor conectividad.
</Callout>
...
Establece `advertise=no` para la dirección IPv6 de WireGuard. No queremos Router Advertisements en el tunnel VPN — los clientes reciben sus direcciones estáticas a través de la configuración de WireGuard, no por SLAAC.
</Callout>
...
La filosofía tradicional de IPv6 desaconseja el NAT, prefiriendo la conectividad extremo a extremo. Sin embargo, NAT66 (masquerade para IPv6) resulta práctico en VPNs que usan direcciones ULA. Tus clientes VPN utilizan direcciones ULA privadas, que se traducen al [prefix IPv6 global asignado por tu ISP](/es/blog/008-mikrotik-pppoe-dualstack-digi/) cuando acceden a internet.
</Callout>
...
WireGuard añade sobrecarga a cada paquete (40 bytes IPv6/20 bytes IPv4 + 8 bytes UDP + 32 bytes cabecera WG). Establecemos `mtu=1500` en la interface WireGuard para permitir paquetes internos de tamaño completo:

- **MTU de la interface WireGuard**: 1500 (paquetes internos; los paquetes externos con sobrecarga WG se fragmentan de forma transparente por la capa de transporte)
...
- **`your-domain.com:53537`** — la IP pública/dominio de tu router y el port de WireGuard
...
<Table title="Parámetros de configuración del cliente">
  ...
  <tbody>
    ...
    <tr>
      <td>`Endpoint`</td>
      <td>La IP pública o nombre de dominio de tu router con el port de WireGuard</td>
    </tr>
    <tr>
      <td>`AllowedIPs`</td>
      <td>`0.0.0.0/0, ::/0` encamina TODO el tráfico por la VPN. Usa subnets específicas para split tunnel</td>
    </tr>
  </tbody>
</Table>
...
<Code lang="routeros" title="Verificar estado de la interface">
```routeros
/interface wireguard print
...
Si tu MikroTik está detrás de otro router o gateway NAT, debes redirigir el port UDP de WireGuard (p. ej., `53537`) a la IP interna de tu MikroTik. WireGuard usa **solo UDP** — la redirección TCP no funcionará.
</Callout>
...
Si usas tu MikroTik como servidor DNS para los clientes VPN (`DNS = 192.168.0.1`), asegúrate de que `allow-remote-requests=yes` está configurado en `/ip dns`. También verifica que la interface WireGuard o su dirección está en la lista de interfaces `LAN`, para que las reglas de input permitan consultas DNS desde los clientes VPN.
</Callout>
...
NAT66 traduce la dirección ULA de tu cliente VPN al prefix IPv6 global de tu ISP. Si tu ISP no proporciona IPv6, no hay prefix global al que traducir, y el acceso a internet por IPv6 no funcionará — el tunnel seguirá transportando IPv6 entre cliente y router, pero no más allá.
</Callout>
...
<li data-check="check">Usa un port no estándar para reducir la exposición a escaneos</li>
