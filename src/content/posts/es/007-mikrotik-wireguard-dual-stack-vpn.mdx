...
La configuración de arriba no es un ejercicio de laboratorio — es el WireGuard que corre hoy en mi propio RB5009UG+S+ con RouterOS 7.23.2, con `wg_devices` en el puerto UDP 53537 exactamente como se usa a lo largo de esta guía, sirviendo a 3 peers configurados con el MTU estándar de 1500.
...
Lo que marca en mi propio router es poco épico, y conviene decirlo con precisión
porque aquí es fácil pasarse. En una ventana de 3 días y 5 horas — el uptime
completo del router, con las dos interfaces WireGuard levantadas todo ese tiempo y
sin una sola caída de enlace — el `cpu-load` se mantuvo entre el 0% y el 5% en diez
muestras, mientras el enlace WAN movía unos 33 Mbps de tráfico agregado. Eso **no**
es un benchmark de WireGuard: en esa misma ventana el túnel movió 312 MB en total,
unos 9 kbps de media, con dos de los tres peers configurados completando handshakes
y uno sin handshake en la ventana medida. Así que lo único honesto que puedo afirmar
es que WireGuard no me cuesta nada medible a esta carga. No tengo una cifra propia
para 100 Mbps a través del túnel, y la única forma de obtenerla es una prueba de
caudal controlada a través de él en tu propio hardware.
