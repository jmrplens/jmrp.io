...
<Callout type="warning" icon="mdi:alert">
  **Nota:** Servir diferente contenido de `robots.txt` según el User-Agent se muestra aquí solo con fines ilustrativos. En la práctica, los crawlers esperan un `robots.txt` consistente y puede que no lo vuelvan a solicitar con diferentes User-Agents. Para el bloqueo real de bots, considera usar `robots.txt` con reglas específicas de User-Agent, o bloqueo del lado del servidor basado en patrones de IP/User-Agent.
</Callout>
