...
faq:
  ...
  - question: "¿Por qué el honeypot solo monitoriza conexiones \"new\"?"
    answer: "El reconocimiento ocurre en el primer packet (el TCP SYN), que lleva el estado \"new\". Apuntar solo a \"new\" evita falsos positivos de sesiones legítimas ya establecidas y minimiza la carga de CPU, ya que solo se inspecciona el sondeo inicial."
  - question: "¿Por qué usar la tabla RAW en lugar de la tabla Filter para bloquear escáneres?"
    answer: "La tabla RAW procesa los packets antes del connection tracking, así que bloquear ahí a los atacantes de la lista negra hace que el router no asigne memoria ni CPU para rastrear sus conexiones. En routers que gestionan miles de ataques diarios esto reduce significativamente el uso de recursos; la detección sigue ejecutándose en la tabla Filter."
...
<Table title="Estados de conexión en RouterOS">
  ...
  <tbody>
    <tr>
      <td>`new`</td>
      <td>Primer packet de un intento de conexión (TCP SYN o primer packet UDP)</td>
    ...
    <tr>
      <td>`invalid`</td>
      <td>Packet que no pertenece a ninguna conexión conocida</td>
      <td>Packets malformados, escaneos de puertos con flags inusuales</td>
    </tr>
  </tbody>
</Table>
...
<Mermaid caption="Handshake TCP de tres vías" maxWidth="500px" ariaLabel="Diagrama de secuencia mostrando un handshake TCP de tres vías: el escáner envía SYN (nueva conexión), el router responde SYN-ACK, el escáner envía ACK para establecer la conexión. El honeypot se activa en el packet SYN inicial.">
```mermaid-render
sequenceDiagram
    participant Escáner
    participant Router
...
flowchart TD
    ...
    H --> I["Descartar packet actual (Filter)"]:::danger
```
</Mermaid>
...
<Table title="Referencia de parámetros de las reglas">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>new</td>
      <td>Solo se activa con el **primer packet** de una conexión (SYN). Previene falsos positivos de sesiones establecidas.</td>
    </tr>
    <tr>
