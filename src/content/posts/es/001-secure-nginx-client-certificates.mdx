...
- **Actívalo en Nginx** con `ssl_client_certificate` (tu CA), `ssl_verify_client on` y `ssl_verify_depth`; nunca uses `optional_no_ca` en producción. Restringe a **TLS 1.2/1.3** con cipher suites ECDHE fuertes.
...
<FileContent filename="Ajustes de Seguridad TLS" icon="mdi:lock">
```nginx
...
<FileContent filename="Control de Acceso en el Bloque Server" icon="devicon:nginx">
```nginx
server {
    # ... SSL configuration ...
...
<FileContent filename="Añadir al bloque server" icon="devicon:nginx">
```nginx
# Enable CRL checking
ssl_crl /etc/nginx/pki/crl/ca.crl;
```
...
<FileContent filename="/usr/local/bin/mtls-add-client.sh" icon="mdi:bash" collapsible={true}>
```bash
#!/bin/bash
set -e
...
<FileContent filename="/usr/local/bin/mtls-renew.sh" icon="mdi:bash" collapsible={true}>
```bash
#!/bin/bash
...
<FileContent filename="crontab -e" icon="mdi:calendar-clock">
```text
