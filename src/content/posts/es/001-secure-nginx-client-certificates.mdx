...
howto:
  ...
  steps:
    ...
    - name: "Crear la certificate authority"
      ...
      anchor: "crear-tu-certificate-authority"
    - name: "Generar certificados de cliente"
      ...
      anchor: "genera-un-certificado-de-cliente"
    - name: "Exportar un paquete PKCS12"
      ...
      anchor: "crea-el-paquete-pkcs12-para-clientes"
    - name: "Configurar Nginx para mTLS"
      ...
      anchor: "configurar-nginx-para-mtls"
    - name: "Añadir revocación de certificados (CRL)"
      ...
      anchor: "revocación-de-certificados-crl-vs-ocsp"
    - name: "Instalar certificados en los dispositivos cliente"
      ...
      anchor: "instalar-certificados-en-los-clientes"
---
...
## ¿Por qué Zero Trust se apoya en mTLS y no en contraseñas?
...
## ¿Qué diferencia hay entre TLS unidireccional y mutual TLS?
...
## ¿Qué ocurre durante un handshake mTLS?

En un handshake mTLS las dos partes demuestran quiénes son: tras el ClientHello y el certificado del servidor, el servidor envía un CertificateRequest, el cliente responde con su propio certificado más una firma CertificateVerify sobre el handshake hasta ese punto, y solo cuando el servidor ha comprobado esa firma contra su CA intercambian claves y empiezan a enviar datos de aplicación. Un cliente sin un certificado firmado por la CA nunca pasa de ese paso.
...
## Crear tu certificate authority



Una CA es la raíz de confianza en tu PKI. Los certificados que emite siguen el perfil [X.509 v3](https://datatracker.ietf.org/doc/html/rfc5280) (RFC 5280), el mismo estándar sobre el que se asienta la PKI pública de la web.
...
### ¿PKCS12 o PEM: qué formato de exportación necesitas?
...
### ¿Qué opción de ssl_verify_client deberías usar?
...
En una configuración moderna solo caben TLS 1.2 y TLS 1.3. [TLS 1.3](https://datatracker.ietf.org/doc/html/rfc8446) (RFC 8446) es el estándar actual, con un handshake más rápido y conjuntos de cifrado exclusivamente AEAD:
...
## ¿Cómo limitar a qué puede acceder cada certificado de cliente?

El acceso se limita comprobando quién dice ser el cliente según su certificado, no solo que sea válido: Nginx expone el subject, el número de serie y la huella del certificado como variables, y un `map` sobre una de ellas (o un `if` dentro de un `location`) decide qué certificados acepta cada ruta, respondiendo 403 a cualquier otro portador de un certificado válido.
