...
lastVerified:
  ...
  versions:
    - RouterOS 7.23.2
    - RB5009UG+S+
description: "Configura PPPoE con DHCPv6 Prefix Delegation en MikroTik RouterOS para DIGI España. VLAN tagging, SLAAC, gestión de cambios de prefijo y firewall."
lead: "DIGI España entrega la sesión PPPoE en la VLAN 20 y delega un /56 por DHCPv6-PD — y el MTU del enlace se negocia a la baja hasta 1480 por LCP, pongas lo que pongas en max-mtu."
...
faq:
  ...
  - question: "¿Qué prefijo IPv6 delega DIGI vía DHCPv6-PD?"
    answer: "DIGI delega un prefijo dinámico /56 mediante DHCPv6 Prefix Delegation, lo que te da 256 subredes /64 para usar en tu LAN principal, IoT, red de invitados y más. El prefijo puede cambiar en cada reconexión."
  - question: "¿Cómo gestiono el cambio de prefijo IPv6 de DIGI en cada reconexión?"
  ...
  - question: "¿Funciona esta configuración de MikroTik con otros ISP además de DIGI?"
    answer: "Sí. Funciona con cualquier ISP basado en PPPoE que proporcione IPv6 vía DHCPv6-PD. Las diferencias entre ISPs suelen ser el ID de VLAN (o ausencia de VLAN), el MTU negociado (normalmente 1480–1492), el formato de usuario y el tamaño del prefijo delegado (/48, /56 o /64)."
topics:
...
howto:
  ...
  steps:
    ...
    - name: "Configurar el cliente DHCPv6 para prefix delegation"
      text: "Añade un cliente DHCPv6 en la interfaz PPPoE solicitando un prefijo hacia pool6, con un script que espera 5 segundos, elimina la dirección anunciada antigua y añade una nueva desde el pool actualizado con advertise=yes."
    ...
    - name: "Verificar la conexión"
      text: "Comprueba el estado del PPPoE, los valores negociados, el prefijo IPv6 delegado en pool6, la dirección asignada a la LAN y prueba la conectividad IPv6 con un ping."
...
<Callout type="info" title="Sobre el prefijo /56">
...
<Mermaid caption="Construcción de la dirección IPv6" ariaLabel="Diagrama que muestra cómo se construye la dirección IPv6 completa combinando el prefijo delegado por el ISP con el sufijo host estático" maxWidth="650px">
```mermaid-render
flowchart LR
...
<li data-check="check">Asegúrate de que pool6 tenga un prefijo válido</li>
</CheckList>
