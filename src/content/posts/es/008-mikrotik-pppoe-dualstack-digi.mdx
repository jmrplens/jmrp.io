...
lead: "DIGI España entrega la sesión PPPoE en la VLAN 20 y delega un /56 por DHCPv6-PD — y la MTU del enlace se negocia a la baja hasta 1480 por LCP, pongas lo que pongas en max-mtu."
...
faq:
  - question: "¿Qué VLAN usa DIGI España para PPPoE?"
    answer: "DIGI España requiere VLAN tagging 802.1Q en la VLAN 20 para el tráfico PPPoE de internet. Si conectas directamente sin VLAN tagging, la autenticación PPPoE fallará."
  - question: "¿Qué prefix IPv6 delega DIGI vía DHCPv6-PD?"
    answer: "DIGI delega un prefix dinámico /56 mediante DHCPv6 Prefix Delegation, lo que te da 256 subnets /64 para usar en tu LAN principal, IoT, red de invitados y más. El prefix puede cambiar en cada reconexión."
  - question: "¿Cómo gestiono el cambio de prefix IPv6 de DIGI en cada reconexión?"
    answer: "Añade un script al cliente DHCPv6 que espere 5 segundos a que se rellene el pool, elimine la dirección anunciada antigua del bridge y añada una nueva desde el pool6 actualizado. Como la dirección usa advertise=yes, RouterOS crea automáticamente el ND prefix para SLAAC."
  - question: "¿Por qué el MTU de PPPoE es 1480 si configuro max-mtu en 1500?"
    answer: "El parámetro max-mtu es solo el límite superior que el cliente acepta, no el MTU real de la conexión. El servidor PPPoE determina el MTU real durante la negociación LCP, y DIGI lo negocia hasta 1480 independientemente del valor de max-mtu."
  - question: "¿Por qué no se autentica mi conexión PPPoE con DIGI?"
    answer: "Comprueba que el VLAN ID sea 20, que el usuario use el formato identificador@digi, que el ONT esté en modo bridge y que el cable físico esté conectado. El cliente DHCPv6 también debe estar en la interfaz PPPoE, no en la VLAN."
  - question: "¿Funciona esta configuración de MikroTik con otros ISP además de DIGI?"
    answer: "Sí. Funciona con cualquier ISP basado en PPPoE que proporcione IPv6 vía DHCPv6-PD. Las diferencias entre ISPs suelen ser el ID de VLAN (o ausencia de VLAN), el MTU negociado (normalmente 1480–1492), el formato de usuario y el tamaño del prefix delegado (/48, /56 o /64)."
topics:
...
howto:
  name: "Configurar dual-stack PPPoE + DHCPv6-PD en MikroTik para DIGI España"
  totalTime: "PT30M"
  tools:
    - "Dispositivo MikroTik RouterOS"
  steps:
    - name: "Configurar la interfaz física"
      text: "Configura el puerto ethernet conectado al ONT de DIGI, habilitando el control de flujo. Usa cualquier puerto no asignado a tu bridge de LAN."
      anchor: "paso-1-configurar-la-interfaz-física"
    - name: "Crear la interfaz VLAN"
      text: "Crea una interfaz VLAN 802.1Q con VLAN ID 20 sobre el puerto físico, ya que DIGI requiere VLAN tagging en la VLAN 20 para el tráfico PPPoE. Omite este paso si tu ISP no usa VLAN tagging."
      anchor: "paso-2-crear-la-interfaz-vlan"
    - name: "Configurar el cliente PPPoE"
      text: "Crea un cliente PPPoE sobre la interfaz VLAN con tus credenciales de DIGI en el formato identificador@digi, habilitando add-default-route y use-peer-dns. El servidor negocia el MTU real a 1480 vía LCP."
      anchor: "paso-3-configurar-el-cliente-pppoe"
    - name: "Añadir las interfaces a la lista WAN"
      text: "Añade el puerto físico, la VLAN y el cliente PPPoE a una lista de interfaces WAN para que las reglas de firewall coincidan correctamente en cada capa de la conexión."
      anchor: "paso-4-añadir-interfaces-a-la-lista-wan"
    - name: "Configurar el cliente DHCPv6 para prefix delegation"
      text: "Añade un cliente DHCPv6 en la interfaz PPPoE solicitando un prefix hacia pool6, con un script que espera 5 segundos, elimina la dirección anunciada antigua y añade una nueva desde el pool actualizado con advertise=yes."
      anchor: "paso-5-configurar-el-cliente-dhcpv6-para-prefix-delegation"
    - name: "Configurar Neighbor Discovery para SLAAC"
      text: "Configura el Neighbor Discovery IPv6 en el bridge con preferencia de RA alta y la dirección link-local del router como DNS, para que los dispositivos LAN autoconfiguren sus direcciones IPv6 vía SLAAC."
      anchor: "paso-7-configurar-neighbor-discovery-slaac"
    - name: "Configurar el NAT IPv4 (masquerade)"
      text: "Añade una regla srcnat masquerade sobre la lista de interfaces WAN para proporcionar conectividad IPv4 saliente a los dispositivos LAN."
      anchor: "paso-8-configurar-nat-ipv4-masquerade"
    - name: "Verificar la conexión"
      text: "Comprueba el estado del PPPoE, los valores negociados, el prefix IPv6 delegado en pool6, la dirección asignada a la LAN y prueba la conectividad IPv6 con un ping."
      anchor: "paso-9-verificar-la-conexión"
---
...
<TLDRSummary title="TL;DR — dual-stack de DIGI España en RouterOS">
  ...
  - **El IPv6 es un `/56` delegado, no un `/64`.** Pide el prefix con DHCPv6-PD y talla un `/64` por cada LAN; apuntar el `/56` directamente a un bridge no funciona.
  - **La MTU del enlace se negocia a la baja hasta 1480 por LCP, pase lo que pase.** `max-mtu=1500` es solo el límite superior del cliente, no la MTU resultante, así que hay que fijar el MSS o media web carga y la otra media se queda colgada.
  - **El prefix delegado cambia al reconectar**, así que hace falta un script DHCPv6 que reescriba las direcciones de la LAN; sin él, el IPv6 muere en silencio tras un corte mientras el IPv4 sigue funcionando.
...
Esto no es teórico: mi propio RB5009UG+S+, con RouterOS 7.23.2, corre exactamente esta configuración de PPPoE + DHCPv6-PD contra DIGI, con la VLAN de WAN etiquetada como `vlan-id=20` (`VLAN_DIGI`) tal y como se describe más arriba.

La gestión del cambio de prefix del Paso 5 no es programación defensiva para un caso raro — es el caso normal. Cada vez que reinicio la interfaz PPPoE, o el router entero, DIGI me asigna un nuevo prefix /56. No de forma ocasional: siempre. Por eso el direccionamiento LAN no puede fijar un prefix a mano y tiene que recalcularlo en cada reconexión; cualquier cosa que asuma un prefix estático se rompe en el primer reinicio. El coste es que las sesiones abiertas se cortan unos segundos mientras la interfaz renegocia — la misma interrupción breve que verías tras cualquier desconexión externa.
