...
lastVerified:
  ...
  versions:
    - RouterOS 7.24
    - RB5009UG+S+
description: "Configure a MikroTik honeypot to detect port scanners, log malicious activity, and auto-block attackers using firewall address lists and the RAW table."
lead: "A RAW-table rule drops a blacklisted scanner before connection tracking allocates anything, so its first reconnaissance packet is also its last. Over 1,000 unique IPs trapped in 24 hours on a domestic line."
...
faq:
  - question: "What is a honeypot?"
    answer: "A honeypot is a security mechanism that creates a decoy system designed to attract and detect attackers, luring them away from legitimate targets while gathering intelligence about their methods. NIST SP 800-53 includes honeypots as a formal security control (SC-26)."
  - question: "Why does the honeypot only monitor \"new\" connections?"
    answer: "Reconnaissance happens on the first packet (the TCP SYN), which carries the \"new\" state. Matching only \"new\" avoids false positives from already-established legitimate sessions and minimizes CPU overhead since only the initial probe is inspected."
  - question: "Why use the RAW table instead of the Filter table to block scanners?"
    answer: "The RAW table processes packets before connection tracking, so blocking blacklisted attackers there means the router never allocates memory or CPU to track their connections. On routers handling thousands of attacks daily this significantly reduces resource usage; detection still runs in the Filter table."
  - question: "How do I avoid blocking myself when deploying the honeypot?"
    answer: "Create a whitelist address-list and add your own IP before enabling any rules. The honeypot rules use src-address-list=!WhiteList so whitelisted IPs are never blocked; if you lock yourself out you'll need physical access to recover."
  - question: "Why are blocked IPs banned for only 4 hours instead of permanently?"
    answer: "The rules use address-list-timeout=4h for a temporary ban. Attacker IPs are often dynamic and reassigned, so permanent bans risk blocking legitimate users later."
  - question: "Does the honeypot work with IPv6?"
    answer: "Yes. The guide provides a separate IPv6 configuration that mirrors IPv4, using a distinct BlackList_PortScanners_v6 address list for comprehensive dual-stack protection."
topics:
  - name: "Honeypot"
    wikidata: "Q911932"
  - name: "MikroTik"
...
howto:
  name: "Build a MikroTik honeypot to trap and auto-block port scanners"
  totalTime: "PT30M"
  tools:
    - "MikroTik RouterOS device"
    - "Linux server running CrowdSec (optional)"
  steps:
    - name: "Create the whitelist"
      text: "Before deploying any blocking rules, create a WhiteList address-list and add your own IPs, VPN endpoints, and trusted services. This prevents accidentally locking yourself out, since the honeypot rules use src-address-list=!WhiteList."
      anchor: "step-1-create-the-whitelist"
    - name: "Deploy the IPv4 honeypot configuration"
      text: "Add the Filter-table detection rules that match connection-state=new on trapped TCP/UDP ports and run action=add-src-to-address-list, then a DROP rule for blacklisted sources. Finish with a RAW prerouting drop rule for high-performance blocking."
      anchor: "step-2-ipv4-honeypot-configuration"
    - name: "Deploy the IPv6 honeypot configuration"
      text: "Apply the mirrored IPv6 ruleset using the separate BlackList_PortScanners_v6 address list for the same trapped ports, providing dual-stack protection across the Filter and RAW tables."
      anchor: "step-3-ipv6-honeypot-configuration"
    - name: "Forward logs to a remote syslog server"
      text: "Set the remote logging action with your syslog server IP and send firewall logs (including the [HONEYPOT] prefixes) so attacker IPs can be collected for threat intelligence."
      anchor: "leveraging-logs-for-threat-intelligence"
    - name: "Integrate the logs with CrowdSec"
      text: "Configure rsyslog to write MikroTik logs to a file, add a CrowdSec acquisition for that file, install the MikroTik parser, and reload CrowdSec to extract attacker IPs automatically."
      anchor: "integrating-with-crowdsec"
    - name: "Test the honeypot"
      text: "From an IP not on the whitelist, connect to a trapped port such as Telnet (23) or SSH, then confirm the IP appears in the BlackList address-list and that the RAW rule drops its traffic. Remove the test IP afterwards."
      anchor: "testing-your-honeypot"
---
...
<TLDRSummary title="TL;DR — MikroTik honeypot in a nutshell">
...
<Mermaid caption="TCP Three-Way Handshake" maxWidth="500px" ariaLabel="Sequence diagram showing a TCP three-way handshake: Scanner sends SYN (new connection), Router replies SYN-ACK, Scanner sends ACK to establish the connection. The honeypot triggers on the initial SYN packet.">
```mermaid-render
sequenceDiagram
    participant Scanner
    participant Router
...
<Mermaid caption="Honeypot Detection Flow" maxWidth="550px" ariaLabel="Flowchart showing the honeypot detection flow: incoming scanner traffic is checked against the blacklist first (RAW table DROP), then checked against trapped ports. Legitimate traffic on ports 80/443 is allowed, while trapped ports trigger logging, syslog, and blacklist addition.">
```mermaid-render
flowchart TD
...
<Table title="TCP Ports for Honeypot Detection">
  ...
  <tbody>
    ...
    <tr><td>8291</td><td>Winbox</td><td>MikroTik</td><td>Router takeover via [known CVEs](https://mikrotik.com/supportsec)</td></tr>
...
<BarChart
  ...
  showPercentage={true}
  showValue={true}
  caption="Data collected over 24 hours on my home connection"
