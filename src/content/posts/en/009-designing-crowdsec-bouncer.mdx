...
<StepByStep title="Decision processing pipeline">
  <li>**Validate required fields** — Ensure `value`, `type`, `duration`, and `scope` are present and non-null. Drop malformed decisions with a warning log.</li>
  <li>**Check decision type** — Most bouncers only handle `ban` decisions. If your bouncer supports `captcha` or custom types, route them accordingly.</li>
  <li>**Check simulation flag** — If `simulated` is `true`, log the decision but do not enforce it. This is useful for testing scenarios.</li>
  <li>**Parse the duration** — Convert the CrowdSec duration string (e.g., `4h0m0s`) to your target system's format. Have a fallback default (e.g., 4 hours) for unparseable durations.</li>
  <li>**Detect IP version** — Determine if the value is IPv4 or IPv6. Check if your target system supports both protocols. Skip unsupported protocols gracefully.</li>
  <li>**Detect address type** — Distinguish single IPs from CIDR ranges (look for `/` in the value). Some systems handle ranges differently from individual addresses.</li>
  <li>**Apply filters** — Check against your configured origin, scope, and scenario filters. Drop decisions that don't match.</li>
  <li>**Transform for target** — Convert the decision into whatever format your target system needs (firewall rule, ACL entry, DNS record).</li>
</StepByStep>
...
<StepByStep title="Integration testing approach">
  <li>**Set up a local CrowdSec instance** — Use Docker or install directly. Configure a test bouncer API key.</li>
  <li>**Create test decisions** — Use `cscli decisions add -i 203.0.113.1 -d 1h -t ban` to manually create decisions.</li>
  <li>**Verify enforcement** — Confirm that the bouncer created the expected rule/entry on the target system.</li>
  <li>**Test deletion** — Use `cscli decisions delete --id X` and verify the bouncer removes the corresponding entry.</li>
  <li>**Test reconciliation** — Manually remove a rule, restart the bouncer, and verify it re-adds the missing entry.</li>
  <li>**Test bulk operations** — Add hundreds of decisions at once and measure startup time.</li>
  <li>**Test failure modes** — Stop the LAPI, restart it, and verify the bouncer recovers gracefully.</li>
</StepByStep>
