...
The configuration above isn't a lab exercise — it's the WireGuard setup running today on my own RB5009UG+S+ on RouterOS 7.23.2, with `wg_devices` on UDP port 53537 exactly as used throughout this guide, serving 3 configured peers at the standard 1500 MTU.
...
What that reads on my own router is undramatic, and worth stating precisely because
it is easy to over-claim here. Over a window of 3 days and 5 hours — the router's
full uptime, with both WireGuard interfaces up the whole time and zero link flaps —
`cpu-load` stayed between 0% and 5% across ten samples while the WAN link carried
roughly 33 Mbps of combined traffic. That is **not** a WireGuard benchmark: in the
same window the tunnel moved 312 MB in total, about 9 kbps on average, with two of
the three configured peers completing handshakes and one showing no handshake within
the measured window. So the honest claim is only that WireGuard costs me nothing
measurable at this load. I have no figure of my own for 100 Mbps through the tunnel,
and the only way to get one is a controlled throughput test across it on your own
hardware.
