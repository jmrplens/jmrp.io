...
The configuration above isn't a lab exercise — it's the WireGuard setup running today on my own RB5009UG+S+ on RouterOS 7.23.2, using the same interface layout as this guide, at the standard 1500 MTU.
...
On my own router `cpu-load` sits in the low single digits with the tunnel up, but I am not going to dress that up as a benchmark: my link is idle most of the time, and a number measured on an idle tunnel tells you nothing about one under load. WireGuard runs in the kernel and is genuinely cheap, but if you need a throughput figure for your own hardware, the only way to get one is a controlled test across the tunnel — `iperf3` between a client and a host on the LAN side, with both ends under your control.
