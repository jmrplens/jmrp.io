...
Two things about that number. The first is that it is a snapshot of that window, not a standing property of the site. Run the same query over the seven complete days from 26 August to 1 September 2026 and the share is **20.1%** — 10,508 of 52,341 — and it swings from day to day: 22.3%, 16.9%, 22.8%, 12.8%, 24.0%, 18.8%, 25.1%. The second is that neither reading can be re-derived later. The Free plan retains `httpRequestsAdaptiveGroups` for eight days and refuses anything older with `cannot request data older than 1w1d`, so both figures are dated observations that the source itself will not serve again. The per-day numbers are here for that reason: they are the only form in which the second measurement stays auditable.
