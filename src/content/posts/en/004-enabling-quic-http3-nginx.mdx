...
Two things about that number. The first is that it is a snapshot of that window, not a standing property of the site. Run the same query over the seven complete UTC days from 26 August to 1 September 2026 and the share is **20.1%** — 10,508 of 52,341 — and it swings from day to day:

<Table title="HTTP/3 share per day — jmrp.io host, 26 August to 1 September 2026">
  <thead>
    <tr>
      <th scope="col">Day (UTC)</th>
      <th scope="col">HTTP/3</th>
      <th scope="col">HTTP/2 + HTTP/3</th>
      <th scope="col">HTTP/3 share</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
The second is that neither reading can be re-derived later. The Free plan retains `httpRequestsAdaptiveGroups` for a rolling eight days and refuses anything older with `cannot request data older than 1w1d`, so both figures are dated observations that the source itself will not serve again. The table is here for that reason, and it publishes both counts rather than only the daily percentage, because the percentages on their own would not be enough: the busiest day carries almost twice the traffic of the quietest, so the seven rounded shares cannot reconstruct the weighted 20.1% — their unweighted mean is 20.4%. With every numerator and denominator on the page, the total is arithmetic anyone can check.
...
server {
    ...
    # No OCSP stapling. Let's Encrypt removed the OCSP URL from its
    # certificates in May 2025 and shut its responders down in August 2025,
    # so with this certificate ssl_stapling only logs "no OCSP responder URL"
    # and does nothing. Keep it only for a CA that still runs OCSP.
...
Stay on HTTP/2 over TCP when UDP is blocked or throttled on the path (corporate firewalls do this), when the server's CPU is the bottleneck (QUIC encrypts in user space and costs more per byte), when your users sit on stable low-loss networks where QUIC's loss recovery has nothing to fix, or when a load balancer in front of Nginx cannot pass UDP through. In each of those cases TCP performs as well and is simpler to run.
