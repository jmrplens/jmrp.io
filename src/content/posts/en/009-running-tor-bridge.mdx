...
- **The consensus flags take days, not minutes.** Running and Valid arrive within the hour; Stable and Guard took about a week on this deployment. They are scored from weighted mean time between failures and, for Guard, bandwidth and familiarity too — not a plain uptime counter — so a bridge restarted every other day never earns them.
...
Two bridges run in Spain, one of them on a mini PC at home in Campello, on the
same domestic fibre line as everything else on the [homelab page](/homelab/) —
...
one in London and one in Spain, because a relay wants stable bandwidth and a
