...
lastVerified:
  date: 2026-08-22
  versions:
    - nginx 1.31.3 (--with-http_perl_module)
...
lead: "A tarpit answers scanners at about 10 bytes per second so they sit there waiting instead of retrying instantly the way they do after a 403 — and a single Brotli setting silently disabled mine for months."
...
howto:
  ...
  steps:
    - name: "Prepare the http block"
      ...
      anchor: "step-1-prepare-nginx-http-block"
    - name: "Create the tarpit snippet"
      ...
      anchor: "step-2-create-the-tarpit-snippet"
    - name: "Define the sensitive paths"
      ...
      anchor: "step-3-define-sensitive-paths-the-traps"
    - name: "Configure the server block"
      ...
      anchor: "step-4-configure-the-server-block"
    - name: "Test the configuration"
      ...
      anchor: "step-5-test-the-configuration"
    - name: "Integrate CrowdSec to auto-ban"
      ...
      anchor: "integration-with-crowdsec"
---
...
<Table title="Same trapped request, before and after the fix">
  <thead>
    <tr>
  ...
  <tbody>
    <tr>
      ...
      <td>0.23 s</td>
      <td>20 s (client timeout)</td>
    </tr>
    <tr>
      <td>Bytes received</td>
      <td>10,255,882</td>
      <td>255,872 (the animation, nothing more)</td>
    </tr>
  </tbody>
</Table>
...
The concept of tarpits in cybersecurity emerged in the late 1990s and early 2000s during a period when network worms and automated scanning tools began proliferating. The most famous early tarpit was **[LaBrea](https://labrea.sourceforge.io/)**, created by [Tom Liston](https://www.giac.org/paper/gsec/1895/labrea-approach-securing-networks/103112) around 2001.
...
If you don't want to create custom parsers, you can use [Fail2Ban](https://github.com/fail2ban/fail2ban) instead. Create a filter that matches lines in `/var/log/nginx/access.tarpit` and configure a jail to ban after N matches.
</Callout>
...
Target known malicious scanners by user-agent. Common scanners include [sqlmap](https://sqlmap.org/) (SQL injection), [Nikto](https://github.com/sullo/nikto) (web vulnerability scanner), [Nmap](https://nmap.org/) (network scanner), [masscan](https://github.com/robertdavidgraham/masscan) (port scanner), and [ZGrab](https://github.com/zmap/zgrab2) (banner grabber):
...
<Callout type="keypoint" title="What this tarpit has actually caught">
The four points above are qualitative, so here is the number. This site runs the
tarpit described here, and the counter is on the [homelab page](/homelab/) —
injected by the edge server as it responds, not by JavaScript, so what you read
is what the server had at that moment. It stood at **1,318 tarpit hits and 90
Nginx bans** when this note was written (2026-08-22); by the time you read it,
the live figure will differ. That is the point: the page is the measurement, not
this sentence.
</Callout>
