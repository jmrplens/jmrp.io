...
Measured against the Mozilla Observatory API, this configuration earns an **A+**. The grade has held across both scoring generations: 140 with all 10 tests passing under algorithm v5, and 160 with all 12 passing under v6, measured 2 September 2026. Neither is the 145 this repository's README advertised until recently, from a December 2025 commit. That older total included a 5-point bonus Observatory grants for serving cookies with `Secure`/`HttpOnly`/`SameSite`; those cookies were two placeholder values that did nothing useful, and removing them — so this site's `/privacy/` page can honestly claim no response ever sets `Set-Cookie` — cost 5 cosmetic points on a policy that already fails nothing. That trade was obviously worth it, and the badge now carries the grade rather than a score, because the grade is the part that doesn't move.
...
## How do XSS attacks work, and how does CSP stop them?

An XSS attack gets the victim's browser to run script the attacker chose inside your page's origin, usually through input the page echoed back without escaping it. CSP stops it because the browser refuses to execute any script the policy does not authorize, so the injected code reaches the page and still never runs.
...
## Which attacks does a CSP actually prevent?
...
## Why are inline scripts the hard part of CSP?

Inline scripts are the hard part because a strict policy blocks every `<script>` block that carries no nonce or hash, and the usual escape hatch, `'unsafe-inline'`, reopens exactly the door XSS walks through. Each inline block therefore has to carry a per-request nonce or move to an external file, and that is the work most sites underestimate.
...
### When should you use `'strict-dynamic'`?
...
## What goes wrong most often when adopting CSP?

The five mistakes that come up most often are allowing `'unsafe-inline'` for scripts, listing sources so broad that they allow the attacker's host too, leaving out a directive the policy needs (`base-uri`, `object-src`, `form-action`), forgetting `always` on the Nginx `add_header` so error pages ship without the header, and losing the header to `add_header` inheritance in a nested block. Each keeps the header on the wire while removing the protection.
...
<Callout type="success" icon="mdi:trophy">
  **Result:** This is exactly the configuration running in production on this site — an A+ rating on [Mozilla Observatory](https://developer.mozilla.org/en-US/observatory), full site functionality intact, and users protected from XSS attacks.
</Callout>
