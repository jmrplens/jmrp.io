...
lead: "94.72% of allowlist-based CSPs can be bypassed, according to Google's own research; a nonce-based policy with strict-dynamic is what actually stops injected script and earns an A+."
...
faq:
  ...
  - question: "Why are nonces better than domain allowlists for CSP?"
    answer: "Google research found that 94.72% of allowlist-based CSPs can be bypassed, because trusted CDNs often host JSONP endpoints, open redirects, or script gadgets that attackers abuse. Nonces are cryptographically random per-request tokens that an attacker cannot guess, so they provide real XSS protection regardless of which libraries are loaded."
...
<TLDRSummary title="TL;DR — a nonce-based CSP on Nginx">
  - **Allowlists do not hold.** Google's own research found **94.72% of allowlist-based CSPs bypassable**, because trusted CDNs host JSONP endpoints and outdated libraries that turn an allowed origin into an execution path.
  - **A nonce is per-request and unguessable**, so an injected `<script>` has no valid token and never runs — which is what an allowlist cannot promise.
  ...
  - **The measured result:** an **A+ on Mozilla Observatory with a score of 140 and all 10 tests passed**, reached by running `Content-Security-Policy-Report-Only` with a `report-uri` for one to two weeks before enforcing.
</TLDRSummary>
...
<Timeline events={[
  ...
  { date: "2016", type: "warning", title: "Google Research", description: "Google publishes 'CSP Is Dead, Long Live CSP' showing 94.72% of allowlist CSPs are bypassable. [Research Paper](https://research.google/pubs/csp-is-dead-long-live-csp-on-the-insecurity-of-whitelists-and-the-future-of-content-security-policy/)" },
