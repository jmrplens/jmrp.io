...
<Callout type="keypoint" icon="mdi:vector-difference">
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Serve robots.txt via alias">
```nginx
server {
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Serve robots.txt via return">
```nginx
server {
    server_name www.example.com;
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Serve security.txt">
```nginx
server {
    # ... server config ...
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Serve humans.txt via alias">
```nginx
location = /humans.txt {
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Serve humans.txt via return">
```nginx
location = /humans.txt {
    return 200 "# TEAM\nLead Developer: Jane Doe\nContact: jane[at]example.com\n\n# SITE\nLast update: 2025-12-18\n";
    add_header Content-Type text/plain;
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Health Check Endpoint">
```nginx
server {
    # ...
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Let's Encrypt Challenge">
```nginx
server {
    # ...
...
<FileContent filename="Nginx Site Configuration" icon="devicon:nginx" ariaLabel="Nginx Config: Mobile App Associations">
```nginx
server {
    # ...
