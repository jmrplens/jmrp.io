...
<Callout type="warning" icon="mdi:alert">
  **Common Mistake:** When using `alias`, always include a trailing slash on both the location and the alias path. Missing slashes cause path resolution errors.
</Callout>
...
<Callout type="info" icon="mdi:information">
  **Note:** Use `default_type` instead of `add_header Content-Type` with `return`. The `add_header` directive may not work as expected in all contexts with `return`.
</Callout>
