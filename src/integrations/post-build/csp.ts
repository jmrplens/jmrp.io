...
export async function finalizeCspConfig(
  ...
  // Firefox prefetch is covered by connect-src 'self'. Chrome still checks the deprecated
  // prefetch-src directive, falling back to default-src 'none' which blocks same-origin
  // prefetches. Adding prefetch-src 'self' explicitly allows them.
...
`;
  ...
  await Promise.all([
    fs.promises.writeFile(path.join(distDir, "security_headers.conf"), content),
    fs.promises.writeFile(
      path.join(distDir, "security_headers_assets.conf"),
      assetsContent,
    ),
  ]);
  logger.info(
    "  ✓ Generated security_headers.conf and security_headers_assets.conf",
  );
}
