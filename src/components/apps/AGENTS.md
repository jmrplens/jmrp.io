...
> 17 standalone tools + 1 internal helper component, ~29,400 lines total. **All use vanilla JS** via `<script is:inline>` — NO Preact.
...
| `BruteForceCost` | 698 | security | `pin-brute-force-calculator` |
...
| `EnvelopeVisualizer` | 1487 | security | `etm-envelope-visualizer` |
...
| `StringPoolPacker` | 1419 | embedded | `string-pool-packer` |
...
Tools are registered in `src/components/pages/ToolPage.astro` (rendered by `src/pages/tools/[...slug].astro`) via a static `componentMap`:
...
1. **security** — CSPBuilder, HashCalculator, CertInspector, HTTPHeadersAnalyzer, PasswordGenerator, EnvelopeVisualizer, BruteForceCost
...
4. **embedded** — ModbusFrameBuilder, StringPoolPacker
...
3. Register in `componentMap` at `src/components/pages/ToolPage.astro`
