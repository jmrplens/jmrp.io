 ...
 * - source-file is a truncated extension name without "://" (some browsers report "chrome-extension" only)
...
function isExtensionViolation(r) {
  ...
  // Match full URIs (moz-extension://...) AND truncated values some browsers
  // report (e.g. "chrome-extension" without the "://").
...
/**
 * Checks whether a CSP violation originates from browser-internal pages
 * (about:blank, about:srcdoc, etc.) or from injected code with no real origin.
 *
 * These violations are generated by:
 * - Browser extensions injecting scripts/styles into about:blank iframes
 * - DevTools-generated code (Overrides, snippets)
 * - Pre-rendering / reader mode frames
 *
 * @param {Record<string, unknown>} r - The csp-report object
 * @returns {boolean} true if the violation comes from a browser-internal source
 */
function isBrowserInternalViolation(r) {
  const rawSource = r["source-file"];
  const source = typeof rawSource === "string" ? rawSource.toLowerCase() : "";

  // about, about:blank, about:srcdoc, about:home, etc.
...
/**
 * Checks whether a CSP violation is a false positive caused by browsers'
 * handling of `prefetch-src`. When Astro's prefetch module creates
 * `<link rel="prefetch">` elements, browsers that lack or mishandle
 * `prefetch-src` fall back to `default-src`, generating spurious violations
 * for same-origin prefetches.
 *
 * Two heuristics are used (both require same-origin blocked-uri):
 * 1. **Known source**: source-file is Astro's prefetch module (`_astro/page.*.js`)
 * 2. **Unknown source**: source-file is null/empty but directive is `default-src`
 *    or `prefetch-src` and blocked-uri is a same-origin HTML page path
 *
 * @param {Record<string, unknown>} r - The csp-report object
...
function processReport(report, ip, ua) {
  ...
  // Discard violations originating from browser-internal pages (about:blank,
  // about:srcdoc) or blob: URIs — these come from extensions/devtools, not
  // from real page content.
  if (isBrowserInternalViolation(r)) {
    return;
  }

  // Silently discard prefetch false positives. Browsers without proper
  // `prefetch-src` support fall back to `default-src`, blocking harmless
  // same-origin prefetches initiated by Astro's prefetch module.
  if (isPrefetchFalsePositive(r)) {
    return;
  }
