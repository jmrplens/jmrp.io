...
themes.forEach((theme) => {
  ...
  if (fs.existsSync(themeDir)) {
      ...
      if (summaryFile) {
          try {
              const content = fs.readFileSync(path.join(themeDir, summaryFile), "utf8");
...
// Status Header
const statusLight = results.light?.failed === 0 ? "✅ Passed" : (results.light ? "❌ Failed" : "—");
const statusDark = results.dark?.failed === 0 ? "✅ Passed" : (results.dark ? "❌ Failed" : "—");

console.log("| Theme | Status | Passed | Failed | Incomplete |");
console.log("| :--- | :--- | :---: | :---: | :---: |");

if (results.light) {
    console.log(`| ☀️ Light | ${statusLight} | ${results.light.passed} | ${results.light.failed} | ${results.light.incomplete} |`);
}
if (results.dark) {
    console.log(`| 🌙 Dark | ${statusDark} | ${results.dark.passed} | ${results.dark.failed} | ${results.dark.incomplete} |`);
}
...
if (hasFailures) {
    ...
    themes.forEach(theme => {
        ...
        if (res && res.failed > 0) {
            console.log(`#### ${theme === 'light' ? '☀️' : '🌙'} ${theme.charAt(0).toUpperCase() + theme.slice(1)} Mode Violations`);
            
            const pages = Array.isArray(res.pages) ? res.pages : [];
            pages.filter(p => p.violations > 0).forEach(p => {
