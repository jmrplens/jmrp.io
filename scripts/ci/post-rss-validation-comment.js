...
` (${report.size} KB)\n`;
      ...
      if (report.metadata.latestItem) {
          comment += `- **Latest:** "${report.metadata.latestItem.title}" (${report.metadata.latestItem.date})\n`;
      }
      ...
      comment += `📥 **Download 
`rss-preview`
 artifact** from the [Workflow Run](https://github.com/${context.repo.owner}/${context.repo.repo}/actions/runs/${context.runId}) to verify styles and components.\n\n`;
