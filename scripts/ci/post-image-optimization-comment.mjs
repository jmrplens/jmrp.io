...
export default async ({ github, context }) => {
  ...
  const jpgCount = execSync('find dist -type f | grep -iE "\.jpe?g$" | wc -l || echo "0"', { encoding: "utf-8" }).trim();

  let largeImagesOutput = "";
  try {
    const largeImages = execSync('find dist -type f -size +500k 2>/dev/null | grep -iE "\.(webp|png|jpe?g)$" || echo ""', { encoding: "utf-8" }).trim();
    if (largeImages) {
